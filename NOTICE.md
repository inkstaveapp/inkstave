...
| Compose UI testing (`org.jetbrains.compose.ui:ui-test` via the `compose.uiTest` Gradle plugin accessor) | 1.12.1 (matches `composeMultiplatform`) | Apache-2.0 | Test-only (`client/shared/src/desktopTest/.../ui/AppUiTest.kt`), closes `docs/testing-strategy.md`'s M1 UI-level e2e gap for the desktop target. Now resolves and compiles; the tests built on it fail at *runtime* with a Skiko native-library issue unrelated to this dependency's own resolution -- see `client/README.md`'s "Known rough edges". |
| CameraX (`androidx.camera:camera-core`/`camera-camera2`/`camera-lifecycle`/`camera-view`) | 1.6.2 (current stable per developer.android.com/jetpack/androidx/releases/camera, verified live) | Apache-2.0 | M4: Android-only in-app camera capture (`client/androidApp/.../CaptureActivity.kt`). Resolved and build-verified in the session this was added (`:androidApp:assembleDebug`, `:androidApp:testDebugUnitTest`). |
| JmDNS (org.jmdns:jmdns) | 3.6.1 (current stable per Maven Central, verified live) | Apache-2.0 | M4: LAN device discovery (`docs/sync-protocol.md`, `client/shared/.../sync/JmDnsSyncDiscovery.kt`). Pure Java, no native code -- works unmodified on both `androidMain` and `desktopMain`. The one genuinely new-dependency-requiring piece of M4's sync slice; there's no JDK-builtin mDNS/DNS-SD support. |
