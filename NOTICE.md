...
| CameraX (`androidx.camera:camera-core`/`camera-camera2`/`camera-lifecycle`/`camera-view`) | 1.6.2 (current stable per developer.android.com/jetpack/androidx/releases/camera, verified live) | Apache-2.0 | M4: Android-only in-app camera capture (`client/androidApp/.../CaptureActivity.kt`). Resolved and build-verified in the session this was added (`:androidApp:assembleDebug`, `:androidApp:testDebugUnitTest`) -- unlike `compose.uiTest` above, this dependency did not hit the JVM-network-restriction issue `client/README.md` documents. |
